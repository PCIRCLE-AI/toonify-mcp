...
- **大型支援內容會變輕** — 大型結構化資料與支援的原始碼，在工作流程往後走之前會先減重
- **小內容通常不會被硬壓** — 如果壓縮不值得，Toonify 會保留原始結果
- **外掛模式維持自動** — 日常 Claude Code 工作流程不需要多一個手動步驟
- **驗證方式很簡單** — `claude plugin list` 應該會看到 `toonify-mcp ✓`
