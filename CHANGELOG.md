...
### Fixed
- Plugin-mode hook now genuinely compresses `Read` and `WebFetch` results instead of never firing on them. `tool_response` for these tools is a structured object (`{type, file:{filePath, content, numLines, startLine, totalLines}}` for Read; `{bytes, code, codeText, result, durationMs, url}` for WebFetch) — not a plain string, so the hook's entry gate had silently no-op'd on every matched call. The hook now extracts the text field per tool, compresses it, and rebuilds the object with only that field replaced, returned via `hookSpecificOutput.updatedToolOutput` — which *replaces* what Claude sees, unlike `additionalContext`, which only appends and made context bigger, not smaller. Both shapes were verified directly and repeatedly: registered this hook via a real Claude Code session's `.claude/settings.json` and read the literal stdin bytes. Measured on a real 500-row JSON file through the actual production code path: 63.8% real token reduction (cl100k_base). A `Read` full-file-content invariant (`numLines === totalLines`) is preserved after compression so Claude isn't misled into thinking there's more of the file left to page in. `Grep` and `Glob` have no adapter: an earlier version of this fix added one for Grep based on a background review agent's claim of having captured its shape live in a separate session; that claim used the same spawning method this repo's own environment was later confirmed (repeatedly, including a fully vanilla `claude -p` invocation with zero custom settings) to never expose a native Grep/Glob tool for, at all — so the claim could not be corroborated and is now considered unreliable rather than merely unverified. The adapter was removed rather than left in on evidence this session no longer trusts; see the comment above `extractText()` in `hooks/post-tool-use.mjs` for the full account. Any `tool_response` this hook can't positively identify — a plain string (older Claude Code, or an unmatched shape) still goes out via `additionalContext`; an unrecognized object (Grep, Glob, anything else) passes through untouched, since there's no safe field to extract from a shape that isn't understood
- Fixed a ReDoS (quadratic backtracking) vulnerability in both the hook and the library's debug-output detector: `hasMultipleFileLocationDiagnostics`'s file-path regex and a sibling stack-frame regex each had an ambiguous character-class/literal overlap that caused O(n²) backtracking on adversarial input (e.g. a long run of `a.a.a.a....` with no valid `:line:col` suffix). Verified before the fix: a single ~120,000-character adversarial line took the real hook process over 10 seconds; the new `MAX_CONTENT_SIZE`/entry-time size guards did not protect against this, since they bound `JSON.parse`/throughput cost, not regex backtracking cost. Fixed structurally by capping the length of any line fed into these heuristics (legitimate debug output never has pathologically long single lines) rather than patching each regex's internal structure, since that approach already missed one instance of the same bug class
...
- Added regression coverage for the object-shaped `tool_response` path: Read/WebFetch compression and reconstruction, the full-read line-count invariant, partial reads, unrecognized tool names, malformed shapes, and Grep/Glob (no adapter for either, by design) all passing through safely
