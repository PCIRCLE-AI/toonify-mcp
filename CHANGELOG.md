...
- Fixed an intermittent `ENOENT: rename ...tmp` failure in `PersistentCache.saveAllAsync` (surfaced as a flaky `lru-cache.test.ts`): the serial write queue and the batch-timer's `flushPendingWrites` (which runs on a `setTimeout`, outside the queue's serialization) both wrote to a single shared `${filePath}.tmp`, so an overlap let one rename the temp away while the other was about to rename the same temp. Each atomic write now uses a unique temp path (pid + counter + random) and cleans up its own temp on failure
...
- Added tests reaching the debug-output *compression* path directly (hook + library) for the ReDoS fix, since the pre-existing ReDoS tests only exercised detection and never invoked the vulnerable compression regexes; added a `showStats` test proving the stats footer never leaks into the replaced `updatedToolOutput` content; added a WebFetch malformed-shape passthrough test and a library-side generic-code-vs-debug-output classification test to close hook/library coverage asymmetries; tightened the Grep `numLines` assertion to a hardcoded expected value instead of re-deriving it with the implementation's own formula
- Fixed a data-integrity bug found in review: with `showStats` on, the `--- Toonify: ... smaller ---` footer was appended to the content *before* it was spliced into `updatedToolOutput`, baking a fake footer into the field Claude treats as the actual file/page content. The footer is now only added on the `additionalContext` (append) path, never the content-replacing path
