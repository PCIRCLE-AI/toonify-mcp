...
透過的な TOON フォーマット変換により、**データ構造に応じて Claude API トークン使用量を 30-65% 削減**し、構造化データでは一般的に **約40%** の節約を実現します。
...
- **30-65% のトークン削減**（通常 約40%）- JSON、CSV、YAML データに対応
...
- 📊 データに裏付けられたトークン節約の主張（30-65% の範囲、通常 約40%）
