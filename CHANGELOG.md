...
### Fixed
- Plugin-mode hook now genuinely compresses `Read` and `WebFetch` results instead of never firing on them. `tool_response` for these tools is a structured object (`{type, file:{filePath, content, numLines, startLine, totalLines}}` for Read; `{bytes, code, codeText, result, durationMs, url}` for WebFetch), verified live against a real Claude Code session — not a plain string, so the hook's entry gate had silently no-op'd on every matched call. The hook now extracts the text field per tool, compresses it, and rebuilds the object with only that field replaced, returned via `hookSpecificOutput.updatedToolOutput` — which *replaces* what Claude sees, unlike `additionalContext`, which only appends and made context bigger, not smaller. Measured on a real 500-row JSON file through the actual production code path: 63.8% real token reduction (cl100k_base). A `Read` full-file-content invariant (`numLines === totalLines`) is preserved after compression so Claude isn't misled into thinking there's more of the file left to page in. Grep/Glob shapes are not yet verified in this build; content from those two tools (and any string `tool_response`, e.g. older Claude Code versions) still goes out via `additionalContext` rather than risk a malformed `updatedToolOutput`
...
- Added regression coverage for the object-shaped `tool_response` path: Read/WebFetch compression and reconstruction, the full-read line-count invariant, partial reads, unrecognized tool names, and malformed shapes all passing through safely
