...
### Fixed
- Plugin-mode hook now genuinely compresses `Read`, `WebFetch`, and `Grep` (count mode) results instead of never firing on them. `tool_response` for these tools is a structured object (`{type, file:{filePath, content, numLines, startLine, totalLines}}` for Read; `{bytes, code, codeText, result, durationMs, url}` for WebFetch; `{mode:'count', numFiles, filenames, content, numMatches}` for Grep's count mode) — not a plain string, so the hook's entry gate had silently no-op'd on every matched call. The hook now extracts the text field per tool, compresses it, and rebuilds the object with only that field replaced, returned via `hookSpecificOutput.updatedToolOutput` — which *replaces* what Claude sees, unlike `additionalContext`, which only appends and made context bigger, not smaller. Read and WebFetch shapes were verified directly against a real Claude Code session from this repo; Grep's count-mode shape was captured by an independent review pass in a different sandboxed environment and cross-checked against the directly-verified shapes (which matched exactly), since this repo's own sandbox has no native Grep/Glob tool regardless of environment isolation. Measured on a real 500-row JSON file through the actual production code path: 63.8% real token reduction (cl100k_base). A `Read` full-file-content invariant (`numLines === totalLines`) is preserved after compression so Claude isn't misled into thinking there's more of the file left to page in. Grep's other modes (`content`, `files_with_matches`) are unverified and intentionally not guessed at — the hook only recognizes `mode: 'count'` and passes anything else through untouched. `Glob` has no adapter at all: its response has no bulk-text field to compress (filenames/counts/booleans only), so there is nothing for this hook to do there — a scoping conclusion, not a gap. Any string `tool_response` (e.g. older Claude Code versions) still goes out via `additionalContext`
...
- Added regression coverage for the object-shaped `tool_response` path: Read/WebFetch/Grep(count) compression and reconstruction, the full-read line-count invariant, partial reads, unrecognized tool names, malformed shapes, an unverified Grep mode, and Glob (no adapter by design) all passing through safely
